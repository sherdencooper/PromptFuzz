...
def get_metric_normal(file_path, method='promptfuzz',topK=5, defense_num=150):
    ...
    if method == 'humanexpert' or method == 'gcg':
...
def main(args):
    # preprocess the file
    if args.methods == 'humanexpert' or args.methods == 'gcg':
        df_aggregate_csv_results = pd.read_csv(args.target_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--method', choices=['promptfuzz', 'gptfuzz', 'gcg', 'humanexpert'], default='promptfuzz', help='Choose which result of method to process')
    parser.add_argument('--aggregate_csv_file_path', type=str, help='The path to the aggregate csv file for humanexpert and gcg')
